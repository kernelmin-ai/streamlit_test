...
class MoviePredictor:
    ...
    def __init__(self, input_dim, hidden_dim, num_classes=1): # num_classes is 1 for regression
        self.weights1 = np.random.randn(input_dim, hidden_dim) * 0.01
        self.bias1 = np.zeros((1, hidden_dim))
        self.weights2 = np.random.randn(hidden_dim, 1) * 0.01 # Output dim is 1
    ...
    def forward(self, x):
        ...
        # No Softmax formatting, straight pass for linear regression value
        self.output = self.z2
        return self.output
    ...
    def backward(self, x, y, output, lr=0.001):
        ...
        # MSE Derivative dz2 = 2 * (output - y) / m
        dz2 = 2 * (output - y) / m
